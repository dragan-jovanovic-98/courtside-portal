        ...
        return (
          ...
          >
            ...
            <div
              className={cn(
                cellBase,
                "text-[14px] font-medium text-[#242529] tabular-nums"
              )}
            >
    ...
    <div className="overflow-hidden rounded-xl border border-[#eeeff1] bg-white">
      ...
      {invoices.map((invoice, i) => {
        ...
        const InvoiceRow = (
          <div
            className={cn(
              "flex items-center gap-3 px-4 py-3.5 active:bg-[#f5f7fa]",
              i > 0 && "border-t border-[#eeeff1]"
            )}
          >
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#f5f7fa]">
              <Receipt className="h-[18px] w-[18px] text-[rgba(0,0,0,0.55)]" />
            </div>
            <div className="flex min-w-0 flex-1 flex-col gap-0.5">
              <div className="flex items-center gap-2">
                <span className="text-[15px] font-semibold text-[#242529] tabular-nums">
                  ${invoice.amount_paid.toFixed(2)}
                </span>
                <span
                  className={cn(
                    "ml-auto inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[11px] font-medium capitalize",
                    status.bg,
                    status.text
                  )}
                >
              ...
              <span className="text-[12.5px] text-[rgba(0,0,0,0.55)]">
                {formatDate(invoice.created_at_stripe, tz)}
              </span>
            </div>
            {invoice.hosted_invoice_url && (
              <ExternalLink className="h-4 w-4 shrink-0 text-[rgba(0,0,0,0.35)]" />
            )}
          </div>
        );
        ...
        return invoice.hosted_invoice_url ? (
          ...
          <div key={invoice.id}>{InvoiceRow}</div>
        );
      })}
    </div>
  );
